...
class ExpeditionsOffersView extends GetView<AllExpeditionsOffersController> {
  @override
              ...
              child: Center(
                  child: BlockButtonWidget(
                    onPressed: () async =>{
                      ...
                      for(var shipping in controller.expeditionOffersSelected){
          ...
          slivers: <Widget>[
                ...
                actions: [
                  Padding(
                      ...
                      child: CircleAvatar(
                        ...
                        child: Obx(() => Center(
                          child: Text(
                              controller.allShippingOffers.length.toString(),
            ...
            SliverToBoxAdapter(
              child: Wrap(
                ...
                LoadingCardWidget() :
                  controller.allShippingOffers.isNotEmpty ?
                  Padding(
                      padding: EdgeInsets.symmetric(horizontal: 10),
                      child: ListView.builder(
                          physics: AlwaysScrollableScrollPhysics(),
                          itemCount: controller.allShippingOffers.length+1 ,
                          shrinkWrap: true,
                          primary: false,

                          itemBuilder: (context, index) {

                            if (index == controller.allShippingOffers.length) {
                              return SizedBox(height: 120);
                            } else {

                              Future.delayed(Duration.zero, (){
                                controller.allShippingOffers.sort((a, b) => b["create_date"].compareTo(a["create_date"]));
                              });
                              return InkWell(
                                  onTap: ()async=>{
                                    controller.expeditionOffersSelected.clear(),
                                    controller.shippingSelected.value = true,
                                  controller.expeditionOffersSelected.add(controller.allShippingOffers[index]),
                                  print('added lenght is: '+controller.expeditionOffersSelected.length.toString()),

                                  },
                                  child: Container(
                                    decoration: BoxDecoration(
                                        border: controller.expeditionOffersSelected.contains(controller.allShippingOffers[index]) ? Border.all(color: interfaceColor,width: 2) : null,
                                        borderRadius: BorderRadius.all(Radius.circular(10))
                                    ),
                                    child: CardWidget(
                                      //selected: controller.shippingSelected.value,
                                      owner: true,
                                        user: controller.allShippingOffers[index]['bool_parcel_reception'] ?
                                        controller.allShippingOffers[index]['parcel_reception_receiver_partner_id'][1] : controller.allShippingOffers[index]['partner_id'][1],
                                        imageUrl: controller.allShippingOffers[index]['bool_parcel_reception'] ?
                                        '${Domain.serverPort}/image/res.partner/${controller.allShippingOffers[index]['parcel_reception_receiver_partner_id'][0]}/image_1920?unique=true&file_response=true' :
                                        '${Domain.serverPort}/image/res.partner/${controller.allShippingOffers[index]['partner_id'][0]}/image_1920?unique=true&file_response=true',
                                      travellerDisagree: controller.allShippingOffers[index]['disagree'],
                                      shippingDate: controller.allShippingOffers[index]['create_date'],
                                      code: controller.allShippingOffers[index]['name'],
                                      travelType: controller.allShippingOffers[index]['booking_type'],
                                      transferable: controller.allShippingOffers[index]['state'].toLowerCase()=='rejected' || controller.allShippingOffers[index]['state'].toLowerCase()=='pending' ? true:false,
                                      bookingState: controller.allShippingOffers[index]['state'],
                                      price: controller.allShippingOffers[index]['shipping_price'],
                                      text: "${controller.allShippingOffers[index]['shipping_departure_city_id'][1].split(" ").first} > ${controller.allShippingOffers[index]['shipping_arrival_city_id'][1].split(" ").first}",
